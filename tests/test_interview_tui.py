...
from tests.fakes import FakePresenter, FakeTask, ScriptedProvider

pytestmark = pytest.mark.tui

TURN_FINISHED = json.dumps({"finished": True, "question": None})
PROPOSAL = json.dumps(
    {
        "task": {"name": "Digest", "description": "d", "user_input": "email me a digest", "prompt": "p"},
        "workflows": [
            {"name": "send", "prompt": "p", "rules": [], "ddl": "CREATE TABLE t (x TEXT)", "cron_expression": None}
        ],
    }
)
...
async def test_interview_to_review_and_approve(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    provider = ScriptedProvider([AgentResult(TURN_FINISHED, ok=True), AgentResult(PROPOSAL, ok=True)])
